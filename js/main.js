...
(function ($) {
  ...
  moive.portfolioFilter = {
    ...
    init: function () {
      ...
      moive.portfolioFilter.container.imagesLoaded(function () {
        ...
        $("#portfolio .content-wrapper .project-filters a").on(
          "click",
          function () {
  ...
  moive.readingIndicator = {
    ...
    init: function () {
      var adjustedHeight =
        moive.pageResolution.containerHeight - moive.pageResolution.pageHeight;
  ...
  moive.skillProgress = {
    ...
    init: function () {
      if (this.completedCounting == 0) {
        this.bottomOfObject =
          $(".single-grouping-skill .single-skill:nth-of-type(1)").offset()
            .top +
          $(
            ".single-grouping-skill .single-skill:nth-of-type(1)"
          ).outerHeight();
        this.bottomOfWindow =
          $(window).scrollTop() + moive.pageResolution.pageHeight;
  ...
  moive.funFactProgress = {
    ...
    count: function () {
      $(".fun-facts .single-fun-fact").each(function () {
        ...
        $(this).find(".fun-fact-value").countTo({
          from: 0,
          to: funFactVal,
          speed: 1000,
        });
    ...
    init: function () {
      if (this.completedCounting == 0) {
        this.bottomOfObject =
          $(".fun-facts .single-fun-fact:nth-of-type(1)").offset().top +
          $(".fun-facts .single-fun-fact:nth-of-type(1)").outerHeight();
        this.bottomOfWindow =
          $(window).scrollTop() + moive.pageResolution.pageHeight;
        if (this.bottomOfWindow > this.bottomOfObject) {
          this.count();
  ...
  moive.processContactForm = function () {
    ...
    var doneFunc = function (response) {
      ...
      message.removeClass("danger-msg").addClass("success-msg").fadeIn();
      setTimeout(function () {
        message.fadeOut();
      }, 3000);
    ...
    var failFunc = function (jqXHR, textStatus, errorThrown) {
      ...
      if (jqXHR.status === 400) {
        ...
        let responseDivTxt = `<ul>`;
        errorData.errors.forEach((e) => {
          responseDivTxt += `<li>`;
          responseDivTxt += `${e.msg}`;
          responseDivTxt += `</li>`;
        });
        responseDivTxt += `</ul>`;
        message.html(responseDivTxt);
      } else {
        message.text(jqXHR.statusText);
      }
      message.removeClass("success-msg").addClass("danger-msg").fadeIn();
      setTimeout(function () {
        message.fadeOut();
      }, 6000);
    };
